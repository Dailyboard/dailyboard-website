<template>
	<div
		id="pricing"
		class="relative isolate bg-background px-6 pt-32 sm:pt-56 lg:px-8"
		data-animate="animate-fade-left"
	>
		<div
			class="absolute inset-x-0 -top-3 -z-10 transform-gpu overflow-hidden px-36 blur-3xl"
			aria-hidden="true"
		>
			<div
				class="mx-auto aspect-[1155/678] w-[72.1875rem] bg-gradient-to-tr from-emerald-400 to-emerald-700 opacity-30"
				style="
					clip-path: polygon(
						74.1% 44.1%,
						100% 61.6%,
						97.5% 26.9%,
						85.5% 0.1%,
						80.7% 2%,
						72.5% 32.5%,
						60.2% 62.4%,
						52.4% 68.1%,
						47.5% 58.3%,
						45.2% 34.5%,
						27.5% 76.7%,
						0.1% 64.9%,
						17.9% 100%,
						27.6% 76.8%,
						76.1% 97.7%,
						74.1% 44.1%
					);
				"
			/>
		</div>
		<div class="mx-auto max-w-2xl text-center lg:max-w-4xl">
			<h2 class="text-base font-semibold leading-7 text-text-primary">Pricing</h2>
			<p class="mt-2 text-2xl font-bold tracking-tight text-text sm:text-4xl">
				Get results or your money back
			</p>
			<p class="mx-auto mt-6 max-w-2xl text-center text-lg leading-8 text-text-weak">
				With Dailyboard you buy your&nbsp;team's&nbsp;time for&nbsp;a&nbsp;fraction
				of&nbsp;its&nbsp;cost. If&nbsp;you&nbsp;don't&nbsp;see results in 60 days, get
				an unconditional refund.
			</p>
		</div>
		<div
			class="mx-auto mt-16 grid max-w-lg grid-cols-1 items-center gap-y-6 sm:mt-20 sm:gap-y-0 lg:grid-cols-1"
		>
			<div
				v-for="(tier, tierIdx) in tiers"
				:key="tier.id"
				:class="[
					tier.featured
						? 'relative bg-background/70 shadow-2xl'
						: 'bg-background/60 sm:mx-8 lg:mx-0',
					tier.featured
						? ''
						: tierIdx === 0
						? 'rounded-t-3xl sm:rounded-b-none lg:rounded-tr-none lg:rounded-bl-3xl'
						: 'sm:rounded-t-none lg:rounded-tr-3xl lg:rounded-bl-none',
					'rounded-3xl p-8 ring-1 ring-gray-900/10 sm:p-10',
				]"
			>
				<h3 :id="tier.id" class="text-base font-semibold leading-7 text-text-primary">
					{{ tier.name }}
				</h3>
				<p class="mt-4 flex items-baseline gap-x-2">
					<span class="text-5xl font-bold tracking-tight text-text">{{
						tier.priceMonthly
					}}</span>
					<span class="text-base text-gray-400">/ user / mo.</span>
				</p>
				<p class="mt-6 text-base leading-7 text-text-weak">{{ tier.description }}</p>
				<ul
					role="list"
					class="mt-8 space-y-3 text-sm leading-6 text-text-weak sm:mt-10"
				>
					<li v-for="feature in tier.features" :key="feature" class="flex gap-x-3">
						<CheckIcon
							class="h-6 w-5 flex-none text-text-primary"
							aria-hidden="true"
						/>
						{{ feature }}
					</li>
				</ul>
				<div class="flex justify-center pt-8 lg:hidden">
					<TryCtaScroll />
				</div>
			</div>
		</div>
	</div>
</template>

<script setup>
	import TryCtaScroll from '../components/TryCtaScroll.vue';
	import { CheckIcon } from '@heroicons/vue/20/solid';

	const tiers = [
		{
			name: 'Pioneers',
			id: 'tier-pioneers',
			priceMonthly: '$5',
			description:
				'Get early access and receive a custom-tailored onboarding and workflow consultation from our team.',
			features: [
				'Custom onboarding experience',
				'Workflow consultations',
				'Unlimited boards',
				'Full commitments history',
			],
			featured: true,
		},
		// {
		// 	name: 'Regular',
		// 	id: 'tier-regular',
		// 	href: '#',
		// 	priceMonthly: '$5',
		// 	description: 'A plan that scales with your rapidly growing business.',
		// 	features: [
		// 		'Priority support',
		// 		'Single sign-on',
		// 		'Enterprise integrations',
		// 		'Custom reporting tools',
		// 	],
		// 	featured: false,
		// },
	];
</script>
